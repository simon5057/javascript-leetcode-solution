/*
 * @lc app=leetcode.cn id=1175 lang=typescript
 *
 * [1175] 质数排列
 *
 * https://leetcode-cn.com/problems/prime-arrangements/description/
 *
 * algorithms
 * Easy (46.95%)
 * Likes:    18
 * Dislikes: 0
 * Total Accepted:    4.2K
 * Total Submissions: 8.9K
 * Testcase Example:  '5'
 *
 * 请你帮忙给从 1 到 n 的数设计排列方案，使得所有的「质数」都应该被放在「质数索引」（索引从 1 开始）上；你需要返回可能的方案总数。
 * 
 * 让我们一起来回顾一下「质数」：质数一定是大于 1 的，并且不能用两个小于它的正整数的乘积来表示。
 * 
 * 由于答案可能会很大，所以请你返回答案 模 mod 10^9 + 7 之后的结果即可。
 * 
 * 
 * 
 * 示例 1：
 * 
 * 输入：n = 5
 * 输出：12
 * 解释：举个例子，[1,2,5,4,3] 是一个有效的排列，但 [5,2,3,4,1] 不是，因为在第二种情况里质数 5 被错误地放在索引为 1
 * 的位置上。
 * 
 * 
 * 示例 2：
 * 
 * 输入：n = 100
 * 输出：682289015
 * 
 * 
 * 
 * 
 * 提示：
 * 
 * 
 * 1 <= n <= 100
 * 
 * 
 */

// @lc code=start
const mod = 10 ** 9 + 7;
function numPrimeArrangements(n: number): number {
    let primeCount = 0;
    for (let i = 2; i <= n; i++) {
        if (isPrime(i)) primeCount++;
    }
    let a = conbin(primeCount, primeCount);
    let b = conbin(n - primeCount, n - primeCount);
    return multi(a, b);
};
// 计算组合数
function conbin(p: number, q: number): number {
    return q === 0 ? 1 : conbin(p, q - 1) * (p - q + 1) % mod;
}
// 是否是素数
function isPrime(n: number): boolean {
    if (n === 2) return true;
    for (let i = 2; i < n; i++) {
        if (n % i === 0) return false;
    }
    return true;
}
// 大数相乘取余(将b拆为两部分来计算)
function multi(a: number, b: number): number {
    let b1 = Math.floor(b / 100000);
    let b2 = b % 100000;
    let sum: number = 0;
    for (let b = 0; b < b1; b++) {
        sum = (sum + a * 100000) % mod;
    }
    sum = (sum + b2 * a) % mod;
    return sum;
}
// @lc code=end

