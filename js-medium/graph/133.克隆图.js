...
var cloneGraph = function (node) {
    ...
    while (queue.length) {
        let cur = queue.shift();
        if (map.has(cur.val)) continue;
        const clone = new Node(cur.val);
        map.set(cur.val, clone);
        cloneQueue.push([clone, cur.neighbors]);
    ...
    let res = cloneQueue[0][0];
    while (cloneQueue.length) {
        let [clone, neighbors] = cloneQueue.shift();
        for (let i = 0; i < neighbors.length; i++) {
            clone.neighbors.push(map.get(neighbors[i].val));
        }
    }
    return res;
};
