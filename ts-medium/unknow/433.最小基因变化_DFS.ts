...
function minMutation(start: string, end: string, bank: string[]): number {
    ...
    function dfs(set: Set<string>, step: number, cur: string, end: string, bank: string[]) {
        if (cur === end) minStep = Math.min(minStep, step);
    ...
    if (minStep === Number.MAX_VALUE) return -1;
    return minStep;
};
// @lc code=end
