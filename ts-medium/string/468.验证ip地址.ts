...
function isIPv4(IP: string) {
    ...
    for (const x of list) {
        if (!x) return false;
        if (x !== '0' && x.startsWith('0')) return false;
        if (/[a-z]|[A-Z]/g.test(x)) return false;
        if (Number(x) > 255) return false;
    }
    return true;
}
function isIPv6(IP: string) {
    ...
    for (const x of list) {
        ...
        for (const ch of x) {
            if (!hexdigits.includes(ch)) return false;
        }
    }
    return true;
}
