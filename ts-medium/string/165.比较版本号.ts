...
function compareVersion(version1: string, version2: string): number {
    const v1List: string[] = version1.split('.');
    const v2List: string[] = version2.split('.');
    for (let i = 0; i < 4; i++) {
        if (Number(v1List[i] || 0) === Number(v2List[i] || 0)) continue;
        if (Number(v1List[i] || 0) > Number(v2List[i] || 0)) {
            return 1;
        }
        return -1;
    }
