...
class MagicDictionary {
    public wordsSet: Set<string>;
    public countsMap: Map<string, number>
    constructor() {
        this.wordsSet = new Set();
        this.countsMap = new Map();
    }
    generateNeighbor(word: string): string[] {
        const list: string[] = [];
        for (let i = 0; i < word.length; i++) {
            list.push(word.substring(0, i) + '*' + word.substring(i + 1));
        }
        return list;
    }
    ...
    buildDict(dictionary: string[]): void {
        for (const word of dictionary) {
            ...
            for (const neighbor of this.generateNeighbor(word)) {
                this.countsMap.set(neighbor, (this.countsMap.get(neighbor) || 0) + 1);
            }
        }
    }

    search(searchWord: string): boolean {
        for (const word of this.generateNeighbor(searchWord)) {
            let count = this.countsMap.get(word);
            if (!count) continue;
            if (count > 1 || count === 1 && !this.wordsSet.has(searchWord)) return true;
        }
        return false;
    }
}
