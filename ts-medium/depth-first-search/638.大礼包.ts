...
function shoppingOffers(price: number[], special: number[][], needs: number[]): number {
    const map = new Map<Array<number>, number>();
    return shopping(price, special, needs, map);
};

function shopping(price: number[], special: number[][], needs: number[], map: Map<Array<number>, number>): number {
